...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        for (int i = 0; i < nodeList.getLength(); i++) {
            ...
            }else if ("tddl".equals(childNode.getNodeName())) { //$NON-NLS-1$
                parseTddl(context, childNode);
            }
        }
    }
    ...
    private void parseTddl(Context context, Node node) {
        JDBCConnectionConfiguration jdbcConnectionConfiguration = new JDBCConnectionConfiguration();

        context.setJdbcConnectionConfiguration(jdbcConnectionConfiguration);

        Properties attributes = parseAttributes(node);
        String driverClass = attributes.getProperty("appName"); //$NON-NLS-1$
        String connectionURL = attributes.getProperty("dbGroupKey"); //$NON-NLS-1$
